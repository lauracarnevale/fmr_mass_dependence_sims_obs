...
if __name__ == "__main__":
    ...
    dtext   = [0,-0.05,-0.1, 0, -0.05]
    dx      = [0, 0, 0, 0.18, 0.18]
    dtx     = [-0.06, 0.25, -0.3, 0.25]
    dty     = [-0.05, 0, -0.5, -0.5]
    dtxa    = [-0.15, 0.25, -0.34, 0.25]
    dtya    = [-0.05, 0, -0.5, -0.5]
    desc    = ['(more', '(fiducial)', '(less', '(SFR > 0)']
    ...
    for l, thresh in enumerate(thresholds):
        ...
        axs[l].axhline(0.0, color='gray', ls='--', alpha=0.5)
    ...
    plt.savefig('./DataGraphs/eta.pdf' , bbox_inches='tight')
